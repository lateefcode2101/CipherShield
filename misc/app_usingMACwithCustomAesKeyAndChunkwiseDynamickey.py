...
def generate_aes_key_with_ecc_equation(vid_or_key, mac_address, first_chunk=True):
    # Generate the x coordinate
    x = int.from_bytes(os.urandom(32), byteorder='big')

    # Convert mac_address to integer
    mac_int = int.from_bytes(mac_address.encode(), 'big')
    ...
    # Compute Keya using the ECC equation
    key_int = (x ** 3 + a * x + mac_int) % (2 ** 256)
    key_bytes = key_int.to_bytes(32, 'big')

    return key_bytes


# def generate_aes_key_with_ecc_equation(vid):
#     ecc_key = generate_integer_from_ecc_equation(vid)
...
def generate_integer_from_ecc_equation(a, b):
    x = int.from_bytes(os.urandom(32), byteorder='big')
    rhs = x**3 + a * x + b
    key = rhs % (2**256)
    return key

def generate_integer_from_ecc_equation(vid):
    global b
...
def encrypt_video(file_path, rsa_public_key, mac_address, output_dir, video_name):
    ...
    aes_key = generate_aes_key_with_ecc_equation(vid, mac_address, first_chunk=True)
    ...
    for idx, chunk in enumerate(chunks):
        ...
        aes_key = generate_aes_key_with_ecc_equation(aes_key, mac_address, first_chunk=False)
