...
def encrypt_video(file_path, rsa_public_key, systemUniqueInformation, output_dir, video_name):
    ...
    return vid, file_size, chunk_time, encryption_time, encrypted_chunks, key_gen_delays, chunk_delays
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            if recipient:
                ...
                vid, file_size, chunk_time, encryption_time,encrypted_chunks,key_gen_delays,chunk_delays = encrypt_video(file_path, public_key,
                ...
                for idx, delay in enumerate(chunk_delays):
                    cur.execute("""
                                INSERT INTO chunk_delays (video_name, chunk_index, delay, recipient_id,video_id)
                                VALUES (%s, %s, %s, %s, %s)
                            """, (video_name, idx, delay, recipient_id,video_id))
                mysql.connection.commit()
                cur.close()
                mysql.connection.commit()
                cur.close()
